...
// Базове hardening HTTP-заголовків (M4). Бекенд віддає JSON API та статичну
// landing page без JavaScript; її CSS завантажується з того самого origin.
// Тому CSP тримаємо дуже вузьким: усе — 'self', без wildcard і без
// 'unsafe-inline'. Це не замінює httpOnly-cookie для токенів (окремий крок, не
// в цьому пасі), але звужує наслідки XSS та клікджекінгу для будь-яких
// відрендерених у браузері відповідей/помилок і майбутньої статики.
//
